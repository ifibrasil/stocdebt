...
To install the package from the online repository, run (if you cannot install directly from GitHub, see the appendix [Installing from downloaded archives](#installing-from-downloaded-archives)).
...
The `<VERSION>` placeholder must be replaced with one of the **stable** version numbers of the package. For example, `"ifibrasil/stocdebt/stocdebt@1.1.2"`. These numbers are on the "Release" list of the online repository (right panel of the website). In case `@<VERSION>` is omitted from the command, a **development** version (subject to errors) of the package will possibly be installed.
...
* public data from the Brazilian Institute of Geography and Statistics (Instituto Brasileiro de Geografia e Estatística - IBGE).
...
```
By setting `subintervals = "2009-12-01/2019-12-01"`, we use only historical values before the Covid-19 pandemic (2020), avoiding observations subject to extreme changes.^[When `case = "ec"`, dates in argument `subintervals` are those of the **first differences** of debt determinants, not of their **levels**. So, for example, if you are dealing with quarterly data and have chosen two nonadjacent `subintervals = c("2011-03-01/2014-12-01","2017-03-01/2019-03-01")`, then shocks will include the difference between values at period `"2017-03-01"` and one period behind, `"2016-12-01"`, even though the latter is not explicitly in `subintervals`.]
...
To generate fan charts in the xlsx file by means of `export_to_excel`, it is necessary that

* there is a Python installation on the computer;
* the following Python modules are installed: `pandas`, `numpy`, `xlsxwriter` and `openpyxl`.

If there is no Python or any of the modules installed, `stocdebt` will warn the user. Even so, it is worth pointing out the following information.
...
The relevant Python functions are triggered from within R, so that there is no need for the user to interact directly with Python. This is true provided that Python is on the system PATH. Usually, when installing Python, it will automatically be added to the PATH. If it is not added, package `stocdebt` will issue a warning. In this case, the user might have to add Python to the system's PATH manually.

Another occasion possibly requiring manual intervention in the PATH is conflict among Python versions. If there is more than one Python version installed on the computer (say, 3.10 and 3.13), the user must be sure that the version detected by R is the one he/she intends to use. R package `reticulate` can be used to find out the version detected by R:
```{r, eval=FALSE}
reticulate::py_discover_config()
```
It is also possible to appoint the desired Python version to R:
```{r, eval=FALSE}
...
# Appendix: installing from downloaded archives {#installing-from-downloaded-archives}

If you cannot install directly from GitHub, download the compressed archives on GitHub [release assets](https://github.com/ifibrasil/stocdebt/releases) (`stocdebt_<VERSION>.zip` for Windows and `stocdebt_<VERSION>.tar.gz` for Linux). For example, `stocdebt_1.1.2.zip` and `stocdebt_1.1.2.tar.gz`.

## On Windows (archive `stocdebt_<VERSION>.zip`)
...
## On Linux (archive `stocdebt_<VERSION>.tar.gz`)

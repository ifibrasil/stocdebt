...
To install the package from the online repository, run (if you cannot install directly from GitHub, see [Installing from downloaded archives](#installing-from-downloaded-archives)).

)download the compressed archives in the release assets (`stocdebt_windows.zip` for Windows and `stocdebt_linux.tar.gz` for Linux), and follow the [instructions to install from them](INSTALL_FROM_ARCHIVES.md).
...
If you cannot install directly from GitHub, download the compressed archives on GitHub [release assets](https://github.com/ifibrasil/stocdebt/releases) (`stocdebt_windows.zip` for Windows and `stocdebt_linux.tar.gz` for Linux).

### On Windows (archive `stocdebt_windows.zip`)
...
## On Linux (archive `stocdebt_linux.tar.gz`)
